...
class CaptionDataLoader(object):
    def __init__(self, dataset, image_feature_root, image_root="", preload_all_features=False, image_mean="imagenet", holding_raw_captions=False):
        ...
        # path to preprocessed image features. It assume the feature are stored with the same name but only extension is changed to .npz
        ...
        if self.preload_all_features:
            self.image_features = np.array([
                np.load("%s/%s.npz" % (self.image_feature_root, os.path.splitext(image["file_path"])[0]))['arr_0']
    ...
    def get_batch(self, batch_size, raw_image=False):
        ...
        if raw_image:
            batch_images = np.array([
                self.image_loader.load(self.image_root + self.images[self.caption2image[i]]["file_path"], expand_batch_dim=False)
                for i in batch_caption_indicies
            ])
        else:
            if self.preload_all_features:
                ...
                batch_images = np.array([
                    np.load("%s/%s.npz" % (self.image_feature_root, os.path.splitext(self.images[self.caption2image[i]]["file_path"])[0]))['arr_0']
                    for i in batch_caption_indicies
                ])
