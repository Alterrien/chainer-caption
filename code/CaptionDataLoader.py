...
class CaptionDataLoader(object):
        ...
        if self.preload_all_features:
            if self.filename_img_id:
                self.image_features = np.array([
                    np.load("%s/%s.npz" % (self.image_feature_path, self.captions[caption_id]["image_id"]))['arr_0']
                    for caption_id in self.caption_ids
                ])
            else:
                self.image_features = np.array([
                    np.load("%s%012d.npz" % (self.image_feature_path, self.captions[caption_id]["image_id"]))['arr_0']
                    for caption_id in self.caption_ids
                ])
    ...
    def get_batch(self, batch_size):
        ...
        if self.preload_all_features:
            ...
            if self.filename_img_id:
                batch_image_features = np.array([
                    np.load("%s/%s.npz" % (self.image_feature_path, self.captions[self.caption_ids[i]]["image_id"]))['arr_0']
                    for i in batch_data_indicies
                ])
            else:
                batch_image_features = np.array([
                    np.load("%s%012d.npz" % (self.image_feature_path, self.captions[self.caption_ids[i]]["image_id"]))['arr_0']
                    for i in batch_data_indicies
                ])
